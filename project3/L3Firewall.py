...
class Firewall(EventMixin):
    def __init__(self, l2config, l3config):
        ...
        # Shore a tuple of MAC pair which will be installed into the flow table of each switch.
    ...
    def installFlow(self, event, offset, srcmac, dstmac, srcip, dstip, sport,
                    dport, nwproto):
        msg = of.ofp_flow_mod()
        match = of.ofp_match()
    ...
    def replyToIP(self, packet, match, event, fwconfig):
        ...
        with open(l3config) as csvfile:
            ...
            for row in self.rules:
                ...
                self.installFlow(event, prio1, srcmac1, dstmac1, s_ip1, d_ip1,
                                 s_port1, d_port1, nw_proto1)
    ...
    def _handle_ConnectionUp(self, event):
        ...
        for (source, destination) in self.disbaled_MAC_pair:
            ...
            print "Applying l2firewall rule:", source, destination
            message = of.ofp_flow_mod(
            )  # OpenFlow massage. Instructs a switch to install a flow
    ...
    def _handle_PacketIn(self, event):
        ...
        s_mac = match.dl_src
        d_mac=packet.dst
        if (not self.match.has_key(str(s_mac))) or (self.match.has_key(str(s_mac)) and self.match[str(s_mac)] != str(s_ip) ):
            self.disbaled_MAC_pair.append(Etheraddr(s_mac),EthAddr(d_mac))
...
def launch(l2config="l2firewall.config", l3config="l3firewall.config"):
    ...
    parser.add_argument('--l2config',
                        action='store',
                        dest='l2config',
                        help='Layer 2 config file',
                        default='l2firewall.config')
    parser.add_argument('--l3config',
                        action='store',
                        dest='l3config',
